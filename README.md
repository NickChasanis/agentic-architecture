...
The ADOPT-001 supervised adoption rehearsal executed all six phases: portable coverage completion, a frozen event-consumer trial contract, two sequential fresh-worker arms, a matched parallel pair from one coordinator, deterministic process-containment and durable restart/backup/restore/rollback on a disposable loopback stack. All local gates closed; the rehearsals do not prove real-model cost, cross-model ranking, remote-host recovery, or production adoption, which remain open.
