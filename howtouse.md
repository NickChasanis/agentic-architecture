...
Our [verification-worker observation](pilot/evaluation/verification-worker-observation.md) exposed a practical pitfall: relative edit paths initially changed the canonical checkout despite an assigned worktree. Require absolute edit targets and explicit command working directories, inspect both checkouts at checkpoints, and use runtime filesystem confinement where available. A written ownership rule and a Git worktree do not enforce each other. Preserve and review accidental artifacts before correcting them.

Only after this delivery path works should you compare cost or concurrency. Use fresh task variants and count planning, troubleshooting, review and integration. Missing token or cost data means unavailable, not free. In the local adoption rehearsal (ADOPT-001), two fresh event-consumer arms ran sequentially and a matched parallel pair ran concurrently from the same coordinator; both routes reached accepted 14/14 oracle results on a small shared task. That capacity observation and delivery rehearsal does not measure cost or generalize capability, and both arms reported the same runtime model identity, so no cross-model ranking follows.

For recovery and staging, name a disposable target and accountable operator first. A restored database or terminated child process is useful local evidence, but neither proves that a worker on another host has stopped. Keep replacement grants blocked until old-writer containment is established, and obtain the owner's target/access/retention decision before external adoption. Public documentation is never deployment authority. The ADOPT-001 rehearsal covered deterministic process termination, durable restart/backup/restore and schema rollback on a loopback-only stack; controlling a real model session or a remote host remains unproven.
