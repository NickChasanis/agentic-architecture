...
The [next implementation roadmap](20-catalog-reliability-and-coordination-recovery.md) gives three examples of evolving the framework. First, an old shop response must not change the current selection or session; test delayed failures as well as successes. Second, pagination must preserve existing consumer promises: a client expecting all products cannot silently receive only the first 25. Introduce an explicit page contract and migrate the consumer. Third, a replacement coordinator must restore complete grants and receipts, then reconcile old processes before granting replacement work.

For example, after restarting on Tuesday, the coordinator may recover Monday's generation-3 task record. It must also recover submission receipts so retries are not processed twice. If it cannot confirm the former writer stopped, it preserves the artifact and blocks replacement. A timestamp, persisted row or tmux window name alone cannot establish that condition. Pagination and durable recovery here are planned capabilities; chapter 20 lists their required proofs.
