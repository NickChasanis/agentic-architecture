...
The foundation, catalog and merchant catalog-list steps are implemented locally. The merchant list adds authenticated shop-scoped draft/published filtering and a contract-validated Angular consumer. Public consumer helpers support title/price filtering and price summaries, including large inputs. The registry validates grants and scopes; recovery tests confirm process termination before reassignment. tmux provides session windows, and the fake adapter remains a test double. A real sequential model-allocation trial is recorded in [the evaluation](../17-recovery-and-model-allocation.md). Durable coordination, real adapter replacement, production pagination and a parallel-worker comparison remain incomplete.
