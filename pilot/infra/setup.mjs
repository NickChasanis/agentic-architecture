...
const ports=localPorts(process.env);
...
const config={fixtureRevision:'foundation-1',origin:`https://localhost:${ports.app}`,
  issuer:`https://localhost:${ports.identity}/realms/agentic-pilot`,clientId:'merchant',clientSecret,
  databaseUrl:`postgres://postgres:${db}@127.0.0.1:${ports.database}/pilot`,
...
write('compose.env',`PILOT_DB_PORT=${ports.database}\nPILOT_IDENTITY_PORT=${ports.identity}\nDB_PASSWORD=${db}\nKC_DB_PASSWORD=${kcdb}\n`);
