...
import {DomainError} from '../identity-tenancy/service.js';
...
export class Catalog {
 ...
 private async authorized(shopId:string,permission:Permission){
  ...
  if(!permission.shopIds.includes(shopId))throw new DomainError('RESOURCE_NOT_FOUND');
 }
 ...
 async get(shopId:string,id:string,permission:Permission){
  ...
  if(!r.rowCount)throw new DomainError('RESOURCE_NOT_FOUND');return merchant(r.rows[0]);
 }
 async update(shopId:string,id:string,input:Draft,permission:Permission){
  ...
  try{await c.query('BEGIN');const r=await c.query('SELECT * FROM catalog.products WHERE id=$1 AND shop_id=$2 FOR UPDATE',[id,shopId]);
   if(!r.rowCount)throw new DomainError('RESOURCE_NOT_FOUND');if(r.rows[0].status==='published')throw new DomainError('PRODUCT_IMMUTABLE');
 ...
 async publish(shopId:string,id:string,permission:Permission){
  await this.authorized(shopId,permission);const c=await this.pool.connect();
  try{await c.query('BEGIN');const r=await c.query('SELECT * FROM catalog.products WHERE id=$1 AND shop_id=$2 FOR UPDATE',[id,shopId]);
   if(!r.rowCount)throw new DomainError('RESOURCE_NOT_FOUND');if(r.rows[0].status==='published'){await c.query('COMMIT');return merchant(r.rows[0]);}
 ...
 async publicList(slug:string){
  ...
  if(!r.rowCount){const shop=await this.pool.query('SELECT 1 FROM shops.shops WHERE slug=$1',[slug]);if(!shop.rowCount)throw new DomainError('RESOURCE_NOT_FOUND');}
 ...
 async publicGet(slug:string,id:string){
  ...
  if(!r.rowCount)throw new DomainError('RESOURCE_NOT_FOUND');return pub(r.rows[0]);
 }
}
